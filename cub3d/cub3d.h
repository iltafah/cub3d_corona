...
typedef	struct	s_bit_map_file_header
{
	char		*file_type;
	int32_t		file_size;
	int32_t		pixel_data_offset;
}				t_bit_map_file_header;

typedef	struct	s_bit_map_info_header
{
...
typedef	struct				s_vals
{
	...
	t_bit_map_file_header	file_header;
	t_bit_map_info_header	info_header;
}							t_vals;

#define		IMG 		vals->img
#define		KEY 		vals->keys
#define		RAY 		vals->ray
#define		CAM 		vals->cam
#define		PLYR 		vals->plyr
#define		MLX 		vals->mlx
#define		CUBFILE 	vals->cubfile
#define		SPRITE 		vals->sprite
#define		TEMP 		vals->temporary
#define		FILE_HEADER	vals->file_header
#define		INFO_HEADER vals->info_header
...
//////////those down are needed orgnize just the top ones
...
void			calculate_sprite_width_height(t_vals *vals, double y_transform);
void			store_sprite_data(t_vals *vals, double y_transform);
